...
THREE.ShaderExtras = {
	...
	'edgedetect': {
		...
		uniforms: {
			...
			tDiffuse: { type: "t", value: 0, texture: null },
			nPixels:    { type: "f", value: 1.5},
			threshhold : {type : 'f', value : 0.2}
		},
		...
		fragmentShader: [
			...
			"float getGray(vec4 c){",
			    "return c.r*0.3333+c.g*0.3333+c.b*0.33333;",
			"}",
			"void main() {",
				"vec2 ox = vec2(nPixels/1024.0, 0.0);",
			    "vec2 oy = vec2(0.0,nPixels/1024.0);",
			    "vec2 uv = vUv.xy;",
			    "vec2 PP = uv - oy;",
			    "vec4 CC = texture2D(tDiffuse,PP-ox); float g00 = getGray(CC);",
			    "CC = texture2D(tDiffuse,PP);    float g01 = getGray(CC);",
			    "CC = texture2D(tDiffuse,PP+ox); float g02 = getGray(CC);",
			    "PP = uv;",
			    "CC = texture2D(tDiffuse,PP-ox); float g10 = getGray(CC);",
			    "CC = texture2D(tDiffuse,PP);    float g11 = getGray(CC);",
			    "CC = texture2D(tDiffuse,PP+ox); float g12 = getGray(CC);",
			    "PP = uv + oy;",
			    "CC = texture2D(tDiffuse,PP-ox); float g20 = getGray(CC);",
			    "CC = texture2D(tDiffuse,PP);    float g21 = getGray(CC);",
			    "CC = texture2D(tDiffuse,PP+ox); float g22 = getGray(CC);",
			    "float K00 = -1.0;",
			    "float K01 = -2.0;",
			    "float K02 = -1.0;",
			    "float K10 = 0.0;",
